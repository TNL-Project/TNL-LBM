...
// using the lattice shear rate u = |W'| (in lattice units). Both compute_analytical_vx()
...
struct NSE_Data_NonNewtonian_Poiseuille : NSE_Data_InflowProfile<TRAITS, DFS_COUNT>, LBM_Data_NonNewtonian<TRAITS>
{};
...
struct StateLocal : State<NSE>
{
	...
	real compute_analytical_vx(idx lbm_z)
	{
		real z_wall_low = 1.5;
	...
	void outputData(UniformDataWriter<TRAITS>& writer, const BLOCK& block, const idx3d& begin, const idx3d& end) override
	{
		...
		writer.write(
			...
			[&](idx x, idx y, idx z) -> dreal
			{
				return TNL::abs(block.hmacro(MACRO::e_vx, x, y, z) - analytical_vx(z));
			},
			begin,
			end
		);
		...
		writer.write(
			...
			[&](idx x, idx y, idx z) -> dreal
			{
				return nse.lat.lbm2physVelocity(analytical_vx(z));
			},
			begin,
			end
		);
		writer.write(
			...
			[&](idx x, idx y, idx z) -> dreal
			{
				return nse.lat.lbm2physVelocity(TNL::abs(block.hmacro(MACRO::e_vx, x, y, z) - analytical_vx(z)));
			},
			begin,
			end
		);
	...
	void probe1() override
	{
		...
		if (nse.rank == 0)
			spdlog::info(
				"at t={:1.6f}s, iterations={:d} l1error_phys_v=[{:e},{:e},{:e}] l2error_phys_v=[{:e},{:e},{:e}] stopping={:e}",
				nse.physTime(),
				nse.iterations,
				l1error_phys_vx,
				l1error_phys_vy,
	...
	if (! use_forcing) {
		...
		for (int j = 0; j < local_y; j++)
			for (int k = 0; k < local_z; k++)
				profile[k * local_y + j] = (dreal) state.analytical_vx(block.offset.z() + k);
#ifdef USE_CUDA
...
)
{
	...
	using NSE_CONFIG = LBM_CONFIG<
		TRAITS,
		D3Q27_KernelStruct,
		NSE_Data_NonNewtonian_Poiseuille,
		COLL,
		typename COLL::EQ,
