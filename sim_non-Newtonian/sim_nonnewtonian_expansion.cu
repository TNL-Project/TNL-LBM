...
struct NSE_Data_NonNewtonian_Expansion : NSE_Data_InflowProfile<TRAITS, DFS_COUNT>, LBM_Data_NonNewtonian<TRAITS>
{};
...
struct StateLocal : State<NSE>
{
	...
	: State<NSE>(id, communicator, std::move(lat), adiosConfigPath, typename TRAITS::bool3d{false, true, false})
	{
		errors_count = 10;
	...
	void setupBoundaries() override
	{
		...
		// Y: periodic (pseudo-2D) — handled by per-dimension periodicity, no map tags
		...
		// 3 m expanded height. Cells with |z_phys| <= 0.5 remain GEO_INFLOW_MOMENT.
...
void run(const std::string& adios_config, int resolution, double lbm_viscosity, double V, double n, double K_phys)
{
	...
	using NSE_CONFIG = LBM_CONFIG<
		TRAITS,
		D3Q27_KernelStruct,
		NSE_Data_NonNewtonian_Expansion,
		COLL,
		typename COLL::EQ,
