...
struct StateLocal_AMR : State_AMR<NSE>
{
	...
	// State_AMR::blockLbmViscosity); the engine hands the functor the GLOBAL
	// site index, so block.offset recovers the local lbm2physPoint
	// coordinate (lat_local.physOrigin already includes the offset)
