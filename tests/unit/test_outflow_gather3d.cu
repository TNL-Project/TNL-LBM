...
static int comp27(int slot, int dim)
{
...
[[maybe_unused]] static int opp27(int slot)
{
...
static double pat(int slot, int x, int y, int z)
{
...
struct GatherMock
{
	...
	__cuda_callable__ double& df(int, int slot, int x, int y, int z) const
	{
...
__global__ void blendKernel3d(const double* in, double* out, int n)
{
...
static void resolveBlends(double* exp, const double* blendA, const double* blendB, const char* isBlend, int n)
{
...
static void computeExpected(int face, bool interp, bool even, int x, int y, int z, double* exp, double* blendA, double* blendB, char* isBlend)
{
	...
	for (int i = 0; i < 27; i++) {
		...
		[[maybe_unused]] auto anchorPat = [&](int slot, int normalOffset) -> double
		{
			int s[3];
...
TEST_CASE("validate-face-detected")
{
	...
	block.hmap.setValue(BC::GEO_WALL);
	block.hmap(x, y, z) = BC::GEO_OUTFLOW_RIGHT_INTERP;
	block.hmap(x, y, z - 1) = BC::GEO_FLUID;
	wire();
	...
	block.hmap(x, y, z) = BC::GEO_OUTFLOW_RIGHT_INTERP;
	block.hmap(x, y, z - 1) = BC::GEO_FLUID;
	block.hmap(x, y, z + 1) = BC::GEO_SYMMETRY;
	wire();
	CHECK_THROWS_AS(block.validateFaceDetectedBC(), std::runtime_error);
	...
	block.hmap.setValue(BC::GEO_WALL);
	block.hmap(0, y, z) = BC::GEO_OUTFLOW_RIGHT_INTERP;
	block.hmap(15, y, z) = BC::GEO_FLUID;
	...
	block.hmap.setValue(BC::GEO_WALL);
	block.hmap(0, y, z) = BC::GEO_OUTFLOW_RIGHT_INTERP;
	block.hmap(1, y, z) = BC::GEO_FLUID;
	wire();
	CHECK_NOTHROW(block.validateFaceDetectedBC());

	// ...and a malformed map (face-BC site in the corner, where all six
	...
	{
		const idx3d sglobal{32, 16, 16};
		const idx3d slocal{16, 16, 16};
		...
		block.hmap.setValue(BC::GEO_WALL);
		block.hmap(16, y, z) = BC::GEO_OUTFLOW_RIGHT_INTERP;
		block.hmap(15, y, z) = BC::GEO_FLUID;
		wire();
		CHECK_NOTHROW(block.validateFaceDetectedBC());
