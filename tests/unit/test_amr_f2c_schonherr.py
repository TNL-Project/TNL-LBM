...
cannot share a binary with the F2C_LAGRAVA opt-out build (ODR hazard on
the kernel template symbol), so the locks live in standalone per-pattern
binaries.  The strategy builds of the ``amr_coupling``
