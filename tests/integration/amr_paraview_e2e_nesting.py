...
Nesting arm of tests/integration/amr_paraview_e2e.py (the amr-nlevel-nesting
plan's commit D): the amr_vtkhdf_writer doctest suite pins the writer's
HDF5 layout on a 3-level chain; this script proves the nested-region file
works for its real consumer. The input is produced by the dedicated driver
tests/regression/test_amr_nesting_sim (a 32^3 periodic Taylor-Green box
with three fine blocks telescoping [6,26) -> [15,25) -> [33,41) per parent
frame) -- sim_AMR/sim_AMR_channel hardcode their region specs, so the e2e
arm drives the mock instead.
...
project root:
    ...
    pvpython tests/integration/amr_paraview_e2e_nesting.py \
