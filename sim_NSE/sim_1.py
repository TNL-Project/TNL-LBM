...
from pytnl_lbm import execute  # noqa: E402


# Define the StateLocal class in Python
class StateLocal(State):
    def __init__(self, id, communicator, lat, adiosConfigPath="adios2.xml"):
    ...
    def outputData(self, writer, block, begin, end):
        hmacro_np = np.from_dlpack(block.hmacro)
        # convert global begin and end to local coordinates so we can pass them along with
        # a local ndarray to the writer.write function
        begin -= block.offset
        end -= block.offset
    ...
    def updateKernelVelocities(self):
...
def sim(RESOLUTION: int = 2, adiosConfigPath: str = "adios2.xml"):
