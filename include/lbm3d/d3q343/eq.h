...
struct D3Q343_EQ
{
	...
	static constexpr const dreal T0 = (dreal)0.6979533220196830882384091; // from closure 105T^3 − 210T^2 + 147T − 36 = 0 in D1Q7
	static constexpr const dreal n1oT0 = 1./T0;
	...
	__cuda_callable__ static dreal feq(dreal rho, int qx, int qy, int qz, dreal vx, dreal vy, dreal vz){
		const dreal ws[4] = {w0,w1,w2,w3};
		const dreal eq = no1 - n1o2 * n1oT0 * (vx * vx + vy * vy + vz * vz) + n1oT0 * (qx * vx + qy * vy + qz * vz)
		+ n1o2 * n1oT0 * n1oT0 * (qx * vx + qy * vy + qz * vz) * (qx * vx + qy * vy + qz * vz);
