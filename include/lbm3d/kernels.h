...
)
{
	if (NSE::BC::isPeriodic(map)) {
		...
		streamGrid.x[NSE::LBM_KS::NoDV] = TNL::min(x, SD.X() - 1 + overlap_x);
		streamGrid.y[NSE::LBM_KS::NoDV] = TNL::min(y, SD.Y() - 1 + overlap_y);
		streamGrid.z[NSE::LBM_KS::NoDV] = TNL::min(z, SD.Z() - 1 + overlap_z);
		for(int i = +; i <= NSE::LBM_KS::NoDV; i++){
