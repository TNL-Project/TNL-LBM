...
void LBM_BLOCK<CONFIG>::setInitialCondition(IC&& ic)
{
	...
	// macroscopic quantities straight from the initial condition (the whole
	// halo-padded range covered by the functor; every ghost row of a shared
	// interface plane holds exactly the value the neighbor's owned write
	// would place there, so the initial exchange reproduces the same bytes;
	// single-rank blocks keep the authored values in their ghost rows)
	TNL::Algorithms::parallelFor<DeviceType>(
		begin,
		end,
		[local_df, SD, ic, goffset] __cuda_callable__(idx3d yzx) mutable
		{
